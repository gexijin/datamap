...
#'   run_datamap()
#' }
run_datamap <- function() {
  # Load required modules
  # Note: No need to source files within the package
  ...
  server <- function(input, output, session) {
    ...
    output$download_example_genomics <- downloadHandler(
      ...
      content = function(file) {
        file.copy(datamap_resource("extdata/Genomics.csv"), file)
      }
    )
    ...
    output$download_example_col_genomics <- downloadHandler(
      ...
      content = function(file) {
        file.copy(datamap_resource("extdata/experiment_design.csv"), file)
      }
    )
    output$download_example_col <- downloadHandler(
      ...
      content = function(file) {
        file.copy(datamap_resource("extdata/iris_column.csv"), file)
      }
    )
    output$download_example_row <- downloadHandler(
      ...
      content = function(file) {
        file.copy(datamap_resource("extdata/Gene_info.csv"), file)
      }
    )
